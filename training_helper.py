import data_processing_final, lang_model_final, file_utils_final
...
def add_alpha_training_vec(train, val, test, n):
    ...
    train_ngram_idxs = data_processing_final.doc_to_ngram_indices(
        train, n, char_to_index)
    val_ngram_idxs = data_processing_final.doc_to_ngram_indices(
        val, n, char_to_index)
    probs, alpha = lang_model_final.train_add_alpha(
        train_ngram_idxs, val_ngram_idxs, alpha_range, n)

    test_ngram_idxs = data_processing_final.doc_to_ngram_indices(
        test, n, char_to_index)
    test_perplexity = lang_model_final.perplexity(test_ngram_idxs, probs)
...
def interp_training_vec(train, val1, val2, n):
    ...
    val2_ngram_idxs = data_processing_final.doc_to_ngram_indices(
    ...
    test_perp = lang_model_final.perplexity(test_ngram_idxs, probs)
    print('******** RESULT ********')
